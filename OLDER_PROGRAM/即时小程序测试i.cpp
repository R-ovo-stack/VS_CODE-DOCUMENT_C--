...
void diaodu::Fcfs()       //先来先服务调度算法
{
    ...
    while(!Queue.empty())
    {
        ...
        p->starttime=stime; //开始执行时间置零,随系统时间改变而变化,上一个进程的结束时间即为下一进程的开始执行时间
...
void diaodu::RR()  //时间片轮转调度算法
{
    ...
    while(!Queue.empty())
    {
         ...
         if(p->needtime>capacity)  //capacity:时间片
         {
             Sleep(capacity);
             p->needtime-=capacity;
             p->runtime+=capacity;
...
void diaodu::HRRN()
{
    ...
    while(!Queue.empty())
    {
        std::list<Pro>::iterator q=Plist.begin(); //头指针   
        for(std::list<Pro>::iterator p=Plist.begin();p!=Plist.end();++p)  //找到响应度最低的进程
            if(p->pri>q->pri)
            { 
                q=p;
            }
...
void diaodu::SRT() 
{
    ...
    {
        std::list<Pro>::iterator q=Plist.begin();    
        for(std::list<Pro>::iterator p=Plist.begin();p!=Plist.end();++p)         //找到剩余时间最短的进程
        {
